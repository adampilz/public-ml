...
CREATE OR REPLACE TABLE `YOUR-BQ-DS.synthetic_data_train` AS
SELECT
...
CREATE OR REPLACE MODEL `YOUR-BQ-DS.xgboost_model`
...
SELECT
  ...
  label
FROM
  `YOUR-BQ-DS.synthetic_data_train`
;


# create a synthetic validation dataset to score using the xgb model
CREATE OR REPLACE TABLE `YOUR-BQ-DS.synthetic_data_validation` AS
SELECT
  ROW_NUMBER() OVER() AS row_id
  , RAND() AS feature1
  , RAND() AS feature2
...
CREATE OR REPLACE TABLE `YOUR-BQ-DS.synthetic_validation_scored` AS
SELECT
    feature1
  , feature2
  , label
...
FROM
  ML.PREDICT(MODEL `YOUR-BQ-DS.xgboost_model`,
    (SELECT *
     FROM `YOUR-BQ-DS.synthetic_data_validation`))
;
...
CREATE OR REPLACE MODEL `YOUR-BQ-DS.probability_calibration_model`
OPTIONS(model_type='LOGISTIC_REG', auto_class_weights=TRUE, input_label_cols=['label']) AS
SELECT
  pos_class_pred_prob
  , label
FROM
  `YOUR-BQ-DS.synthetic_validation_scored`
;
...
CREATE OR REPLACE TABLE `YOUR-BQ-DS.calibrated_probabilities` AS
SELECT
  pos_class_pred_prob
  , predicted_label_probs[SAFE_OFFSET(0)].prob AS calibrated_pos_class_pred_prob
FROM
  ML.PREDICT(MODEL `YOUR-BQ-DS.probability_calibration_model`,
    (SELECT *
...
FROM `YOUR-BQ-DS.calibrated_probabilities`
;
